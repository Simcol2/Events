...
// Mixed-checkout purchase lines are tagged with source_kind/source_id
// metadata on their inline product_data (see create-unified-checkout-
// session.js) - the booking and security deposit lines are not. That tag
// is what separates "an actual purchased item" from "a rental deposit"
// inside one Stripe session, so the purchase order never gets the
// deposits added into its total or listed as line items.
async function syncMixedPurchaseOrder(supabase, session, invoice, { customerId, purchaseSubtotalCents }) {
  const email = session.customer_details?.email || session.customer_email;
  const name = session.customer_details?.name || null;
  const customer = customerId ? { id: customerId } : await findOrCreateCustomer(supabase, email, name);
  ...
  const purchaseLines = lineItems.data.filter((line) => {
    const product = line.price?.product;
    return Boolean(typeof product === "object" && product?.metadata?.source_kind);
  });

  const totalCents = purchaseLines.reduce((sum, line) => sum + (line.amount_total || 0), 0) || purchaseSubtotalCents;
  const subtotalCents =
    purchaseLines.reduce((sum, line) => sum + (line.amount_subtotal ?? line.amount_total ?? 0), 0) ||
...
async function syncMixedCheckout(supabase, session) {
  ...
  if (purchaseSubtotalCents > 0) {
    await syncMixedPurchaseOrder(supabase, session, invoice, { customerId, purchaseSubtotalCents });
  }
}
