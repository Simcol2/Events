  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div>
        ...
        <input value={form.tagline} onChange={set("tagline")} className="mt-1.5 w-full rounded-sm border border-[#D9D9D9] px-3 py-2.5 font-[Space_Grotesk] text-sm outline-none focus:border-[#0B4933]" />
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input required type="number" min="0" step="0.01" value={form.price} onChange={set("price")} className="mt-1.5 w-full rounded-sm border border-[#D9D9D9] px-3 py-2.5 font-[Space_Grotesk] text-sm outline-none focus:border-[#0B4933]" />
        </div>
        <div>
          <label className="block font-[Space_Grotesk] text-[10px] font-semibold tracking-[0.12em] text-[#0B4933]">CUSTOM PRICE</label>
          <input type="number" min="0" step="0.01" value={form.custom_price} onChange={set("custom_price")} placeholder="Only if different from price" className="mt-1.5 w-full rounded-sm border border-[#D9D9D9] px-3 py-2.5 font-[Space_Grotesk] text-sm outline-none focus:border-[#0B4933]" />
        </div>
      </div>
      ...
      {form.customizable && (
        <div className="rounded-sm border border-[#E6E6E6] p-4">
          <label className="flex cursor-pointer items-center gap-2">
            ...
            {form.options.map((opt, i) => (
              <div key={i} className="flex items-start gap-2">
                <input
                  ...
                  className="w-1/3 rounded-sm border border-[#D9D9D9] px-3 py-2 font-[Space_Grotesk] text-xs outline-none focus:border-[#0B4933]"
                />
                <input
                  value={opt.photo_url}
                  onChange={(e) => updateOption(i, "photo_url", e.target.value)}
                  placeholder="Photo URL"
                  className="flex-1 rounded-sm border border-[#D9D9D9] px-3 py-2 font-[Space_Grotesk] text-xs outline-none focus:border-[#0B4933]"
                />
