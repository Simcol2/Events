...
export default function AdminItemForm({ item, onSave, onCancel, onDelete, saving }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div>
        <label className="block font-[Space_Grotesk] text-[10px] font-semibold tracking-[0.12em] text-[#0B4933]">TAGS</label>
        <div className="mt-2 grid grid-cols-2 gap-x-4 gap-y-2 sm:grid-cols-3">
          {TAGS.map((tag) => (
            <label key={tag.id} className="flex cursor-pointer items-center gap-2">
              <input type="checkbox" checked={tagIds.includes(tag.id)} onChange={() => toggleTag(tag.id)} className="h-3.5 w-3.5 accent-[#0B4933]" />
              <span className="font-[Space_Grotesk] text-xs text-[#5C5645]">{tag.label}</span>
            </label>
          ))}
        </div>
      </div>
