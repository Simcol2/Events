  ...
  return (
    <div className="overflow-hidden bg-white">
      ...
      <div className="px-1 pb-3 pt-4">
        <h3 className="font-['Fraunces'] text-[25px] font-semibold leading-[1] text-[#0B4933]">{name}</h3>
        {tagline && <p className="mt-1 font-[Space_Grotesk] text-sm italic text-[#8A6A1E]">{tagline}</p>}
        {description && <p className="mt-2 font-[Space_Grotesk] text-sm leading-5 text-[#5C5645]">{description}</p>}
        <div className="mt-3 flex items-center justify-between border-t border-[#EAE3D3] pt-3">
          ...
          {onCustomize ? (
            <button
              onClick={onCustomize}
              className="flex items-center gap-1.5 rounded-full px-4 py-2 font-[Space_Grotesk] text-sm font-semibold tracking-[0.14em]"
              style={{ background: "transparent", color: "#0B4933", border: "1px solid #0B4933" }}
            >
              SELECT CARD
            </button>
          ) : (
            <button
              ...
              className="flex items-center gap-1.5 rounded-full px-4 py-2 font-[Space_Grotesk] text-sm font-semibold tracking-[0.14em]"
              style={{
                background: inCart ? "#0B4933" : "transparent",
                color: inCart ? "#FFFFFF" : "#0B4933",
    ...
    <div>
      <section className="border-b border-[#EAE3D3]">
        <div className="mx-auto max-w-7xl px-5 py-16 sm:px-8">
          <div className="flex items-center justify-between">
            <div>
              <p className="font-[Space_Grotesk] text-sm font-semibold tracking-[0.3em] text-[#8A6A1E]">GIFTS</p>
              <h1 className="mt-2 font-['Fraunces'] text-4xl font-semibold leading-[1.05] text-[#0B4933] sm:text-[42px]">
                The games are played. The memories are made.
              </h1>
              <p className="mt-3 max-w-xl font-[Space_Grotesk] text-base leading-6 text-[#8C846F]">
            ...
            <button
              ...
              className="hidden items-center gap-2 font-[Space_Grotesk] text-sm font-semibold tracking-[0.1em] text-[#0B4933] sm:flex"
            >
              <ShoppingBag size={16} />
              CART ({cartCount})
            </button>
      ...
      {checkoutStatus === "success" && (
        <div className="mx-auto mt-6 max-w-7xl px-5 sm:px-8">
          <div className="rounded-sm border border-[#8A6A1E] bg-[#F0F3EA] px-5 py-4 font-[Space_Grotesk] text-base text-[#0B4933]">
      ...
      {checkoutStatus === "cancelled" && (
        <div className="mx-auto mt-6 max-w-7xl px-5 sm:px-8">
          <div className="rounded-sm border border-[#D8D0BC] bg-white px-5 py-4 font-[Space_Grotesk] text-base text-[#8C846F]">
      ...
      <section className="mx-auto max-w-7xl px-5 py-14 sm:px-8">
        <div className="mb-6 flex items-center justify-between sm:hidden">
          <button
            onClick={() => setShowCart(true)}
            className="flex items-center gap-2 font-[Space_Grotesk] text-sm font-semibold tracking-[0.1em] text-[#0B4933]"
          >
            <ShoppingBag size={16} />
            CART ({cartCount})
          </button>
        ...
        <h2 className="mb-2 font-['Fraunces'] text-2xl font-semibold text-[#0B4933]">Guest Gifts</h2>
        <p className="mb-6 max-w-2xl font-[Space_Grotesk] text-base leading-6 text-[#8C846F]">
          ...
          {KEEPSAKES.map((k) => {
            ...
            return (
              <div key={k.id} className="overflow-hidden bg-white">
                ...
                <div className="px-1 pb-3 pt-4">
                  <h3 className="font-['Fraunces'] text-[25px] font-semibold leading-[1] text-[#0B4933]">{name}</h3>
                  <p className="mt-1 font-[Space_Grotesk] text-sm italic text-[#8A6A1E]">{k.tagline}</p>
                  <p className="mt-2 font-[Space_Grotesk] text-sm leading-5 text-[#5C5645]">{k.description}</p>
                  <div className="mt-3 flex items-center justify-between border-t border-[#EAE3D3] pt-3">
                    <span className="font-[Space_Grotesk] text-sm font-medium tracking-[0.08em] text-[#8A6A1E]">
        ...
        {gifts.length > 0 && (
          <>
            <h2 className="mb-6 font-['Fraunces'] text-2xl font-semibold text-[#0B4933]">Just for You</h2>
            <p className="mb-6 max-w-2xl font-[Space_Grotesk] text-base leading-6 text-[#8C846F]">
        ...
        {giftsError && <p className="mb-14 font-[Space_Grotesk] text-base text-red-700">Couldn't load gifts: {giftsError}</p>}

        {wrapAndStationeryItems.length > 0 && (
          <>
            <h2 className="mb-6 font-['Fraunces'] text-2xl font-semibold text-[#0B4933]">Gift Wrap & Stationery</h2>
            <p className="mb-6 max-w-2xl font-[Space_Grotesk] text-base leading-6 text-[#8C846F]">
        ...
        <h2 className="mb-6 font-['Fraunces'] text-2xl font-semibold text-[#0B4933]">Keepsakes & Gifts</h2>
        {loading && <p className="py-10 text-center font-[Space_Grotesk] text-base text-[#5A5F54]">Curating the collection...</p>}
        {error && <p className="py-10 text-center font-[Space_Grotesk] text-base text-red-700">Couldn't load the collection: {error}</p>}
        {!loading && !error && giftItems.length === 0 && (
          <p className="py-10 text-center font-[Space_Grotesk] text-base text-[#5A5F54]">Nothing tagged yet, check back soon.</p>
        )}
