...
const ESSENTIALS_FEATURES = [
  { id: "guessArrival", name: "Guess the Arrival Website" },
  { id: "pictureThis", name: "Picture This Installation & Keepsake" },
  { id: "ohBabyCenterpiece", name: "Oh Baby Centerpiece" },
  { id: "babyTrivia", name: "Baby Trivia Card Pack" },
  { id: "nurseryRhyme", name: "Custom Nursery Rhyme Artwork" },
  { id: "welcomeSign", name: "Welcome Sign" },
];

const ADDONS = [
  { id: "photoWall", name: "Photo Wall Installation", price: 350 },
  { id: "voiceNotes", name: "Voice Notes", price: 300 },
  { id: "digitalAlbum", name: "Digital Memory Album", price: 100 },
];
...
export default function Home({ navigate }) {
  ...
  return (
    <div className="overflow-hidden" style={{ background: palette.bg }}>
          ...
          <div className="grid items-center gap-12 lg:grid-cols-[0.9fr_1.1fr] lg:gap-16">
            <Reveal className="relative z-10">
              ...
              <p className="mt-7 max-w-xl text-lg leading-8" style={{ ...fonts.bodyFont, color: palette.ink }}>
                One package. Everything included. Decor, a personalized digital experience, and
                guest activities — built around the people at the center of your day.
              </p>
      ...
      {/* THE ESSENTIALS — the actual product, front and center */}
        ...
        <Reveal>
          <div className="text-center mb-14">
            ...
            <p className="mt-3 max-w-xl mx-auto text-lg leading-8" style={{ ...fonts.bodyFont, color: palette.ink }}>
              The core experience — everything you need for a shower guests will talk about long after it's over.
            </p>
          </div>
        </Reveal>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {ESSENTIALS_FEATURES.map((f, i) => (
            <Reveal key={f.id} delay={i * 60}>
              <div className="rounded-xl overflow-hidden shadow-sm" style={{ background: palette.surface, border: `1px solid ${palette.line}` }}>
                <PhotoSlot photoKey={f.id} label={f.name} />
                <div className="p-5 flex items-start gap-2">
                  <Check size={18} color={palette.accent} className="mt-0.5 flex-shrink-0" />
                  <p className="text-base font-semibold" style={{ ...fonts.bodyFont, color: palette.ink }}>{f.name}</p>
                </div>
              </div>
            </Reveal>
          ))}
          ...
          <Reveal>
            <div className="text-center mb-12">
              ...
              <p className="mt-3 text-lg" style={{ ...fonts.bodyFont, color: palette.ink }}>
                Or get all three bundled in with It Lasts Forever, our complete experience.
              </p>
            </div>
          </Reveal>
          <div className="grid gap-6 sm:grid-cols-3">
            {ADDONS.map((a, i) => (
              <Reveal key={a.id} delay={i * 80}>
                <div className="rounded-xl overflow-hidden shadow-sm" style={{ background: palette.surface, border: `1px solid ${palette.line}` }}>
                  <PhotoSlot photoKey={a.id} label={a.name} />
                  <div className="p-5">
                    <p className="text-base font-semibold" style={{ ...fonts.bodyFont, color: palette.ink }}>{a.name}</p>
                    <p className="mt-1 text-lg font-bold" style={{ ...fonts.displayFont, color: palette.accent }}>+${a.price}</p>
                  </div>
                </div>
              </Reveal>
            ))}
